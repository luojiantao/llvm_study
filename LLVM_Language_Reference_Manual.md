...
- “coldcc” - 冷调用约定
    ...
    这个调用约定企图假设在不通常的调用中尽可能高效的生成代码.例如,这些调用通常保存所有的寄存器，以便在调用中，这个调用不会中断任何活动范围。这个调用约定不支持可变参数并且要求所有调用方的原型精确匹配函数定义的原型.此外inliner不考虑这这样的函数调用直接插入.
    
- “cc 10” - GHC约定

  这个调用约定已经被专门为了使用通过Haskell编译器实现。在寄存器中计入了所有东西，通过禁用调用方保存寄存器来使目标成功。这个调用约定不应该轻易的使用除非是特别场景，例如另一种方案当实现函数式编程语言时经常使用的寄存器固定性能技术。现在只有X86支持这个约定并且有下面这些限制：

  - 在X86-32只支持最多4bit类型参数。不支持floating-point 类型
  - 在X86-64上，只支持最多10bit类型参数和6浮点(floating-point)参数

	这个调用约定支持尾调用优化但是要求调用者和被调用者都要使用它。
